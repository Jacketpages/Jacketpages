...
(1, 'Created', 'The bill has just been created.'),
...
SELECT ID, 1, GTUSERNAME, PHONE, HOME_PHONE_NUMBER, EMAIL, '', FIRST_NAME, LAST_NAME, LEVEL, 1, 1, NOW(), NULL, NULL
...
SELECT ID, NAME, DESCRIPTION, 0, STATUS, DUES, NULL, UCASE(SHORT_NAME),
WEBSITE, ORGANIZATION_EMAIL, 1, PHONE_NUMBER, FAX_NUMBER, 0, ANNUAL_EVENTS, 0, CHARTER_DATE,
ELECTIONS, MEETING_INFO, MEETING_FREQUENCY, NULL
...
INSERT INTO JACKETPAGES.BILLS (ID, TITLE, DESCRIPTION, CREATE_DATE, SUBMIT_DATE, DUES, FUNDRAISING, NUMBER, TYPE, CATEGORY, JFC_RECOMMENDATIONS, SUBMITTER, ORG_ID, GSS_ID, GCC_ID, UGMEMBERS, GMEMBERS)
SELECT ID, TITLE, DESCRIPTION, CREATE_DATE, SUBMIT_DATE, DUES, FUNDRAISING, NUMBER, TYPE, CATEGORY, JFC_RECOMMENDATIONS, USER_ID, ORGANIZATION_ID, 1, 1, UGMEMBERS, GMEMBERS
...
UPDATE JACKETPAGES.BILLS B
INNER JOIN JACKETPAGES.BILL_AUTHORS V
ON B.ID = V.BILL_ID
SET B.AUTH_ID = V.ID;

ALTER TABLE BILL_AUTHORS DROP BILL_ID;
...
SELECT ID, 1, BILL_ID, PARENT_ID, STATE, NAME, COSTPERUNIT, QUANTITY, TOTALCOST, AMOUNT, ACCOUNT,  'General', NULL, 0, 0, NOW()
