...
class MembershipsController extends AppController {
	...
	public function index($id = null) {
		...
		$officers = $this -> Membership -> find('all', array('conditions' => array('AND' => array('Membership.org_id' => $id, 'Membership.role <>' => 'Member', 'OR' => array($db -> expression('Membership.end_date >= NOW()'), 'Membership.end_date' => '0000-00-00'))), ));

		$members = $this -> Membership -> find('all', array('conditions' => array('AND' => array('Membership.org_id' => $id, 'Membership.role' => 'Member', 'OR' => array($db -> expression('Membership.end_date >= NOW()'), 'Membership.end_date' => '0000-00-00'))), ));
		$pending_members = $this -> Membership -> find('all', array('conditions' => array('AND' => array('Membership.status' => 'Pending', 'Membership.org_id' => $id))));
	...
	public function accept($id = null, $orgId = null) {
		if (!$id) {
			$this -> Session -> setFlash(__('Invalid ID for membership', true));
			$this -> redirect(array('controller' => 'memberships', 'action' => 'index'));
		}
		$this -> Membership -> id = $id;
